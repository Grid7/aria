...
이걸 보고 진짜 소름이 돋았습니다. 예전에는 회의 끝나고 한 시간씩 붙잡고 있던 정리 작업이 단 5초 만에 끝나버렸거든요. 물론 AI가 뽑아준 내용을 그대로 믿으면 안 되고 한 번씩 검수는 해줘야 합니다. 가끔 엉뚱한 사람한테 업무를 할당하는 헛발질을 하기도 하니까요.
...
진짜 맥북 터지는 줄 알고 식겁해서 전원 버튼 꾹 눌러서 강제 종료했습니다. 재부팅하고 나서 다시 해봐도 글자 하나 출력하는 데 1분이 넘게 걸리더라고요. 내가 뭘 잘못했나 싶어서 구글링을 미친 듯이 해봤습니다. 알고 보니 제 맥북 램이 8기가밖에 안 되는데, 70B 모델은 최소 64기가 램은 있어야 원활하게 돌아가는 괴물이었던 겁니다. 제 경차에 덤프트럭 엔진을 욱여넣으려고 했으니 컴퓨터가 비명을 지를 수밖에요.
...
그렇게 세팅을 끝내고 다시 말을 걸어보니, 이번에는 챗GPT 못지않은 속도로 답변이 쫙쫙 뽑혀 나오더라고요. 심지어 인터넷 연결을 끊어도 작동하니까 보안 걱정 없이 회사 내부 문서 요약시키는 데 아주 찰떡이었습니다. 물론 70B 모델만큼 엄청나게 복잡한 추론을 해내진 못하지만, 일상적인 코딩 질문이나 이메일 초안 작성 정도는 차고 넘치게 잘 해냅니다.
