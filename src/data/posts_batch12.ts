...
<img src="https://picsum.photos/seed/midjourney-fail/800/400" alt="Midjourney Fail" referrerPolicy="no-referrer" />

## 1. 시드(Seed) 값의 함정: 왜 얼굴이 계속 바뀔까?

처음에는 시드(Seed) 값을 고정하면 된다는 유튜브 영상을 보고 그대로 따라 해봤습니다. 시드 번호를 복사해서 파라미터에 넣고 돌렸죠. 근데 웬걸, 얼굴 각도만 조금 바뀌어도 완전히 다른 사람처럼 보이는 겁니다. 마치 닮은꼴 사촌 형제들만 잔뜩 소환하는 기분이었어요. 진짜 이때 마우스 집어 던질 뻔했습니다. 프롬프트 깎는 노인이라는 말이 왜 나왔는지 뼈저리게 느끼면서 주말 내내 모니터만 노려봤네요. 

시드 값은 사실 전체적인 이미지의 '초기 노이즈 패턴'을 고정하는 역할을 합니다. 즉, 구도나 전반적인 분위기, 색감 등을 비슷하게 유지하는 데는 탁월하지만, 특정 인물의 이목구비나 미세한 특징을 완벽하게 고정하는 데는 한계가 명확합니다. 프롬프트에 '웃는 표정'이나 '달리는 포즈' 같은 새로운 변수가 추가되면, 기존의 노이즈 패턴이 깨지면서 캐릭터의 얼굴도 미묘하게 틀어지게 되는 것이죠. 이런 시행착오는 [로컬 AI 설치 과정](/blog/ollama-local-llm-installation-guide)에서 겪었던 기술적 한계와도 비슷하게 느껴졌습니다.

<img src="https://picsum.photos/seed/midjourney-seed/800/400" alt="Midjourney Seed" referrerPolicy="no-referrer" />

## 2. 구원투수 등장: 캐릭터 레퍼런스(--cref) 기능

그러다 우연히 디스코드 커뮤니티에서 '캐릭터 레퍼런스(Character Reference)'라는 기능을 알게 됐습니다. 제가 완전히 헛다리를 짚고 있었던 거죠. 시드 값은 전체적인 이미지의 분위기나 구도를 고정하는 데 더 적합한 거고, 특정 인물의 얼굴을 유지하려면 아예 다른 파라미터를 써야 했던 겁니다.

바로 \`--cref\` 파라미터를 적용해봤습니다. 기준이 될 캐릭터 이미지를 하나 딱 정해서 링크를 따고, 프롬프트 맨 뒤에 하이픈 두 개 긋고 cref를 붙인 다음 링크를 넣었죠. 그리고 캐릭터 가중치를 조절하는 cw 파라미터까지 100으로 꽉 채워서 돌려봤습니다. 

결과는 진짜 충격적이었습니다. 아까까지 중구난방이던 캐릭터가 갑자기 똑같은 얼굴로 옷만 갈아입고 나오더라고요. 심지어 표정을 찡그리거나 웃게 만들어도 이목구비의 특징이 그대로 유지됐습니다. \`--cw 100\`을 주면 얼굴뿐만 아니라 헤어스타일과 옷차림까지 최대한 비슷하게 가져오고, \`--cw 0\`으로 설정하면 오직 '얼굴'에만 집중해서 다른 옷이나 배경을 자유롭게 바꿀 수 있다는 꿀팁도 얻었습니다.

<img src="https://picsum.photos/seed/midjourney-cref-success/800/400" alt="Midjourney Cref Success" referrerPolicy="no-referrer" />

## 3. 완벽한 캐릭터 시트를 위한 실전 팁

물론 완벽하진 않습니다. 가끔 손가락이 기괴하게 나오거나 배경이랑 인물이 따로 노는 현상은 여전하더라고요. 그래도 예전처럼 매번 가챠 돌리듯이 기도하면서 엔터 키를 누르던 시절에 비하면 진짜 천국이 따로 없습니다. 

성공적인 캐릭터 고정을 위해서는 다음 세 가지를 꼭 기억하세요:
1. **선명하고 특징이 뚜렷한 기준 이미지(Reference Image)를 사용할 것.** 정면, 측면 등 다양한 각도의 이미지를 여러 장 섞어서 \`--cref URL1 URL2\` 형태로 넣으면 정확도가 훨씬 올라갑니다.
2. **스타일 레퍼런스(--sref)와 혼동하지 말 것.** \`--sref\`는 화풍이나 그림체를 고정하는 것이고, \`--cref\`는 인물을 고정하는 것입니다. 두 개를 적절히 섞어 쓰면 나만의 독특한 웹툰이나 동화책을 만드는 것도 가능합니다.
3. **후보정 작업의 중요성.** 생성된 이미지의 퀄리티를 높이기 위해 포토샵이나 다른 [최신 AI 도구들](/blog/remote-work-productivity-tools-2026)을 활용해 후보정 작업을 거치는 것이 좋습니다. AI가 그린 그림을 그대로 쓰기보다는, 사람의 손길이 한 번 더 닿았을 때 비로소 완성도가 높아집니다.

개인적으로 이제 캐릭터 일관성 유지하려고 시드 값에 집착하는 건 완전 비추합니다. 무조건 레퍼런스 이미지 확실하게 하나 뽑아두고 cref 기능으로 밀어붙이는 게 정신 건강에 훨씬 좋습니다. 앞으로 AI 이미지를 활용해 콘텐츠를 만드실 분들은 이 기능을 꼭 마스터하시길 바랍니다. 시간 낭비를 획기적으로 줄여줄 거예요.
...
<img src="https://picsum.photos/seed/notion-ai-blank/800/400" alt="Notion AI Blank Page" referrerPolicy="no-referrer" />

## 1. 챗GPT처럼 쓰려다 맞은 참사

처음에는 그냥 빈 페이지에 대충 회의했던 내용 키워드 몇 개 던져주고 요약해달라고 해봤습니다. 근데 얘가 문맥을 전혀 못 잡더라고요. A팀장이 화내면서 지시한 중요한 안건은 쏙 빼먹고, 지나가듯 농담한 내용을 핵심 과제랍시고 정리해놓는 걸 보고 진짜 어이가 없었습니다. 돈 아까워서 바로 구독 취소 버튼 누르러 갈 뻔했어요. 챗GPT 무료 버전보다 못한 걸 내가 왜 돈 주고 쓰고 있나 자괴감이 들더라고요. 

이런 도구 활용의 미숙함은 [원격 근무 생산성 도구들](/blog/remote-work-productivity-tools-2026)을 처음 도입했을 때의 혼란과도 비슷했습니다. 챗GPT는 방대한 외부 지식을 바탕으로 대화하는 챗봇이라면, 노션 AI는 철저하게 '내 워크스페이스 안의 데이터'를 가공하는 데 특화된 도구라는 사실을 간과했던 것입니다. 빈 페이지에서 허공에 대고 질문을 하니까 엉뚱한 소리를 했던 거죠.

<img src="https://picsum.photos/seed/notion-database/800/400" alt="Notion Database" referrerPolicy="no-referrer" />

## 2. 데이터베이스 속성: 노션 AI의 진짜 무기

며칠 씩씩대다가 문득 노션 AI의 진짜 용도를 제가 오해하고 있다는 생각이 들었습니다. 그래서 방식을 완전히 바꿨습니다. 빈 페이지에 글을 쓰는 대신, 데이터베이스를 먼저 빡세게 구축했죠. 

회의록 템플릿을 만들고, 그 안에 'AI 요약(AI Summary)' 속성과 'AI 후속 작업 추출(AI Action Items)' 속성을 아예 박아버렸습니다. 그리고 회의가 끝나면 클로바노트 같은 앱으로 녹음한 텍스트 변환본을 그냥 본문에 통째로 복사해서 붙여넣기만 했어요. 

그랬더니 마법 같은 일이 벌어졌습니다. 데이터베이스 목록 화면에서 각 회의마다 핵심 요약이랑 누가 언제까지 뭘 해야 하는지 액션 아이템이 자동으로 쫙 뽑혀 나오는 겁니다. 제가 일일이 프롬프트를 칠 필요도 없이, 페이지가 생성되고 내용이 채워지는 순간 AI가 백그라운드에서 알아서 데이터를 정리해주는 시스템이 완성된 것이죠.

<img src="https://picsum.photos/seed/notion-automation/800/400" alt="Notion Automation" referrerPolicy="no-referrer" />
...
노션 AI를 200% 활용하기 위한 몇 가지 팁을 공유합니다:
1. **AI 자동 채우기 속성 적극 활용:** '번역', '핵심 키워드 추출', '맞춤형 프롬프트' 등 데이터베이스 속성에서 제공하는 AI 기능을 활용해 반복적인 텍스트 처리 작업을 자동화하세요.
2. **Q&A 기능으로 사내 위키 구축:** 노션 Q&A 기능을 사용하면 워크스페이스 전체를 검색해서 답변을 찾아줍니다. "지난달 마케팅 예산 기안서 어디 있어?"라고 물어보면 해당 문서를 찾아 요약까지 해줍니다.
3. **템플릿과 결합하기:** 매주 작성하는 주간 보고서나 업무 일지 템플릿에 AI 블록을 미리 세팅해두면, 버튼 클릭 한 번으로 초안을 완성할 수 있습니다.
...
<img src="https://picsum.photos/seed/ollama-install/800/400" alt="Ollama Installation" referrerPolicy="no-referrer" />

## 1. 덤프트럭 엔진을 경차에 달다: 하드웨어의 한계

유튜브 보니까 올라마(Ollama)라는 프로그램만 깔면 맥북에서도 엄청 똑똑한 AI를 공짜로 쓸 수 있다고 하더라고요. 그래서 당장 다운받고 터미널 열어서 가장 성능이 좋다는 라마 3(Llama 3) 70B 모델을 설치하라고 명령어를 쳤습니다. 다운로드하는 데만 한참 걸리길래 넷플릭스 한 편 보고 왔죠. 

드디어 설치가 끝나고 안녕이라고 인사를 건넸는데... 컴퓨터가 갑자기 비행기 이륙하는 소리를 내더니 화면이 그대로 멈춰버렸습니다. 진짜 맥북 터지는 줄 알고 식겁해서 전원 버튼 꾹 눌러서 강제 종료했습니다. 재부팅하고 나서 다시 해봐도 글자 하나 출력하는 데 1분이 넘게 걸리더라고요. 

내가 뭘 잘못했나 싶어서 구글링을 미친 듯이 해봤습니다. 알고 보니 제 맥북 램(RAM)이 8기가밖에 안 되는데, 70B 모델은 최소 64기가 램은 있어야 원활하게 돌아가는 괴물이었던 겁니다. 제 경차에 덤프트럭 엔진을 욱여넣으려고 했으니 컴퓨터가 비명을 지를 수밖에요. 이런 하드웨어의 한계는 미드저니의 고사양 작업을 할 때와는 또 다른 차원의 고통이었습니다.

<img src="https://picsum.photos/seed/ollama-models/800/400" alt="Ollama Models" referrerPolicy="no-referrer" />

## 2. 현실적인 타협안: 8B 모델과 Web UI의 조합

결국 눈물을 머금고 그 무거운 모델을 싹 지워버렸습니다. 대신 제 컴퓨터 사양에 맞는 8B 모델로 다시 설치했죠. 파라미터 수가 적어지면 성능이 크게 떨어질까 걱정했지만, 일상적인 용도로는 충분히 훌륭한 퍼포먼스를 보여주었습니다.

그리고 터미널 까만 화면에서 타자 치는 게 너무 불편해서, 애니띵LLM(AnythingLLM)이나 Open WebUI 같은 웹 UI 프로그램도 연동해줬습니다. 이렇게 세팅하니까 마치 챗GPT 웹사이트를 쓰는 것과 똑같은 깔끔한 화면에서 로컬 AI와 대화할 수 있게 되었습니다.

그렇게 세팅을 끝내고 다시 말을 걸어보니, 이번에는 챗GPT 못지않은 속도로 답변이 쫙쫙 뽑혀 나오더라고요. 심지어 인터넷 연결을 끊어도 작동하니까 보안 걱정 없이 회사 내부 문서 요약시키는 데 아주 찰떡이었습니다. 특히 개인 보안이 중요한 작업을 할 때 로컬 AI는 최고의 선택지라는 걸 깨달았습니다.

<img src="https://picsum.photos/seed/ollama-security/800/400" alt="Ollama Security" referrerPolicy="no-referrer" />

## 3. 로컬 AI, 누구에게 필요할까?

물론 8B 모델이 70B 모델이나 GPT-4만큼 엄청나게 복잡한 추론을 해내진 못합니다. 하지만 일상적인 코딩 질문이나 이메일 초안 작성, 텍스트 요약 정도는 차고 넘치게 잘 해냅니다.

로컬 AI 도입을 고민 중이시라면 다음 사항을 체크해보세요:
* **보안이 최우선인가?** 회사 기밀 문서나 개인정보가 포함된 데이터를 다뤄야 한다면 외부 서버로 데이터가 전송되지 않는 로컬 AI가 필수입니다.
* **하드웨어 사양이 충분한가?** 최소 16GB, 권장 32GB 이상의 RAM과 넉넉한 VRAM을 가진 GPU가 있어야 쾌적한 사용이 가능합니다.
* **오프라인 환경인가?** 비행기 안이나 인터넷 연결이 불안정한 곳에서도 언제든 AI 비서를 활용할 수 있습니다.

본인 컴퓨터 사양도 모르고 무조건 파라미터 수 높은 모델부터 다운받는 건 절대 비추합니다. 시간 낭비에 컴퓨터 수명만 깎아먹는 지름길이에요. 자기 램 용량에 맞춰서 적당히 타협한 모델 고르고, 예쁜 UI 프로그램 하나 얹어서 쓰는 게 로컬 AI를 가장 스트레스 없이 즐기는 방법입니다.
